...
class Base64Encoder:
    """Handles Base64 encoding/decoding of PowerShell scripts."""
    
    @staticmethod
    def encode(script_text: str, compression: bool = True) -> tuple[str, str]:
        """
        Encode a PowerShell script using Base64 with optional Gzip compression.
        
        Args:
            script_text (str): The script to encode
            compression (bool): Whether to compress before encoding
            
        Returns:
            tuple[str, str]: (encoded_script, decoder_stub)
        """
        try:
            ...
            if compression:
                ...
                with gzip.GzipFile(fileobj=compressed, mode='wb') as gz:
                    gz.write(script_bytes)
                encoded = base64.b64encode(compressed.getvalue()).decode('utf-8')
            else:
                encoded = base64.b64encode(script_bytes).decode('utf-8')
            
            # Create the decoder stub
            decoder = f'''# Anti-analysis checks
...
try {{
    ...
    @staticmethod
    def decode(encoded_text: str) -> Optional[str]:
        """
        Decode a Base64 encoded PowerShell script.
        
        Args:
            encoded_text (str): The encoded script
            
        Returns:
            Optional[str]: The decoded script, or None if decoding fails
        """
        try:
            # Decode Base64
            ...
            try:
                with gzip.GzipFile(fileobj=BytesIO(decoded)) as gz:
